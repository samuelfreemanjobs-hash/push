...
export default function HoursManager({ userId }) {
  ...
  async function handleSave() {
    setSaving(true);
    await setDoc(doc(db, 'businesses', userId), { hours }, { merge: true });
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm max-w-lg">
        ...
        {DAYS.map((day) => (
          <div key={day} className="flex items-center gap-4">
            <label className="flex items-center gap-2 w-32 cursor-pointer">
              ...
              />
              <span className={`text-sm font-medium ${hours[day].enabled ? 'text-gray-900' : 'text-gray-400'}`}>
                {LABELS[day]}
              </span>
            </label>
            {hours[day].enabled ? (
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="mt-8 bg-indigo-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50"
      >
